...
const SALT: [u8; 16] = [
    0x79, 0x6f, 0x6c, 0x6e, 0x6f, 0x6d, 0x61, 0x5f, 0x73, 0x74, 0x65, 0x61, 0x6d, 0x5f, 0x6b, 0x79,
];
...
pub fn referer() -> Result<String, String> {
    ...
    let key = decode_obfuscated(OBFUSCATED)
        .ok_or_else(|| "VITE_ABC_KEY was not embedded during the Rust build".to_string())?
        .replace('_', "");

    const INDICES: [usize; 15] = [3, 3, 3, 15, 10, 11, 8, 12, 15, 8, 17, 11, 13, 0, 0];
    let bytes = key.as_bytes();
    if bytes.iter().any(|byte| !byte.is_ascii())
        || INDICES[..13].iter().any(|&index| index >= bytes.len())
    {
    ...
    Ok(format!(
        "https://{}.{}.{}/",
        first_label, second_label, top_level_domain
    ))
}
