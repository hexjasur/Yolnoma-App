...
fn truncate_diff(value: String) -> String {
    if value.chars().count() <= MAX_DIFF_CHARS { return value; }
...
fn untracked_diff(root: &Path, relative_path: &str) -> String {
    ...
    let Ok(bytes) = std::fs::read(&file) else { return "[Could not read new file]".to_string(); };
    if bytes.contains(&0) { return "[Binary file omitted]".to_string(); }
...
pub fn get_git_changes(root_path: String) -> Result<Vec<GitChange>, String> {
    let root = PathBuf::from(root_path).canonicalize().map_err(|error| format!("Could not access selected folder: {error}"))?;
    if !root.is_dir() { return Err("The selected path is not a folder.".to_string()); }
...
pub fn get_git_history(root_path: String, limit: Option<u32>) -> Result<Vec<GitCommit>, String> {
    ...
    let output = run_git(&root, &["log", "--all", "--date=short", &count_arg, "--pretty=format:%H%x1f%h%x1f%an%x1f%ad%x1f%s%x1f%P%x1e"])?;
    if !output.status.success() {
        let message = String::from_utf8_lossy(&output.stderr).trim().to_string();
        return Err(if message.is_empty() { "Could not read Git history.".to_string() } else { message });
    }
    let refs_output = run_git(&root, &["show-ref"])?;
    let mut refs_by_hash: std::collections::HashMap<String, Vec<String>> = std::collections::HashMap::new();
    for line in String::from_utf8_lossy(&refs_output.stdout).lines() {
        let Some((hash, name)) = line.split_once(' ') else { continue; };
        let short_name = name.strip_prefix("refs/heads/").or_else(|| name.strip_prefix("refs/tags/")).unwrap_or(name);
        refs_by_hash.entry(hash.to_string()).or_default().push(short_name.to_string());
    }
    let mut commits = Vec::new();
    for record in String::from_utf8_lossy(&output.stdout).split('\x1e').filter(|record| !record.is_empty()) {
