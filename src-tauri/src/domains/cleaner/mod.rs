...
const CLEAR_AMD_CACHE: &str = include_script!("amd-cache.ps1");
...
const CLEAN_BUN_CACHE: &str = include_script!("bun-cache.ps1");
...
fn script_for_action(action: &str) -> Option<&'static str> {
    match action {
        ...
        "amd-cache" => Some(CLEAR_AMD_CACHE),
        ...
        "bun-cache" => Some(CLEAN_BUN_CACHE),
        _ => None,
    }
}
