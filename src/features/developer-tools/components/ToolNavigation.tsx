    ...
    <aside className="border border-white/[0.08] bg-[#111109] p-2">
      ...
      <nav className="flex flex-wrap gap-1">
        {items.map(([id, label, Icon]) => (
          <button key={id} type="button" onClick={() => onChange(id)} className={`flex min-w-[150px] flex-1 items-center gap-3 border-l-2 px-4 py-3 text-left text-sm transition ${active === id ? 'border-[var(--accent)] bg-[var(--accent-glow)] text-white' : 'border-transparent text-white/50 hover:bg-white/[0.04] hover:text-white'}`}>
