...
export function responseError(response: ProxyResponse, fallback = 'OpenRouter request failed.') {
  return response.body.error?.message ?? `${fallback} (HTTP ${response.status})`;
}

export async function fetchOpenRouterModels(apiKey: string) {
  const response = await invoke<ProxyResponse>('proxy_request', {
    method: 'GET', url: `${OPENROUTER_BASE}/models`,
...
export function useOpenRouterSession(userId: string, defaults: OpenRouterModel[]) {
  ...
  useEffect(() => {
    ...
    setModelsError('');
    void fetchOpenRouterModels(apiKey).then(({ response, models: fetched }) => {
      if (cancelled) return;
      if (response.status < 200 || response.status >= 300 || !fetched.length) {
        setModelsError(responseError(response, 'Model list unavailable'));
        return;
      }
      const featured = new Set(defaults.map((model) => model.id));
      const ordered = [...fetched.filter((model) => featured.has(model.id)), ...fetched.filter((model) => !featured.has(model.id))];
      setModels(ordered);
      setSelectedModel((current) => ordered.some((model) => model.id === current) ? current : ordered[0]?.id ?? '');
    }).catch((error) => { if (!cancelled) setModelsError(error instanceof Error ? error.message : String(error)); })
  ...
  return { apiKey, draftKey, setDraftKey, apiKeyReady, models, modelsLoading, modelsError, selectedModel, setSelectedModel, saveKey };
}

export function requestChatCompletion(apiKey: string, model: string, messages: ChatMessage[], systemContext: string) {
  return requestOpenRouter({ apiKey, model, messages, systemContext, title: 'Yolnoma AI Chat' });
}

export function listChatSessions(userId: string) { return invoke<ChatSessionSummary[]>('list_ai_chat_sessions', { userId }); }
export function getChatSession(userId: string, sessionId: string) { return invoke<ChatSession>('get_ai_chat_session', { userId, sessionId }); }
export function createChatSession(userId: string, title = 'New chat') { return invoke<ChatSession>('create_ai_chat_session', { userId, title }); }
export function saveChatSession(userId: string, session: ChatSession) { return invoke<void>('save_ai_chat_session', { userId, session }); }
export function deleteChatSession(userId: string, sessionId: string) { return invoke<void>('delete_ai_chat_session', { userId, sessionId }); }

export async function generateChatTitle(apiKey: string, model: string, prompt: string) {
  const response = await requestOpenRouter({ apiKey, model, maxTokens: 24, title: 'Yolnoma AI Chat', messages: [{ role: 'user', content: prompt.slice(0, 500) }], systemContext: 'Create a concise 3-6 word title for the conversation. Reply with only the title, no quotes, punctuation, or explanation.' });
