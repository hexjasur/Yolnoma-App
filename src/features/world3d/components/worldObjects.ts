...
export const WORLD_NODES: WorldNode[] = WORLD_TOOLS.map((tool, index) => {
...
export function createTextSprite(text: string, color: string, scale = 1) {
...
export function createNode(node: WorldNode) {
  const group = new THREE.Group();
...
export function createWoodenTable() {
...
export function createSittingFigure() {
