...
export default function World3DPage() {
  ...
  const selectedLabel = selected ? NODE_LABELS[selected] : null;
  const openNode = () => { if (selected) navigate(NODE_ROUTES[selected]); };

  if (!isStandalone) {
    return <div className="mx-auto flex min-h-[calc(100vh-8rem)] max-w-3xl items-center justify-center px-6"><div className="w-full rounded-[2rem] border border-white/10 bg-gradient-to-br from-[#102331] to-[#111109] p-8 text-center shadow-2xl md:p-12"><div className="mx-auto flex h-20 w-20 items-center justify-center rounded-3xl bg-amber-200/10 text-amber-200"><Trees size={38} /></div><p className="mt-6 text-[11px] font-semibold uppercase tracking-[0.25em] text-amber-200/75">Yolnoma World</p><h1 className="mt-3 font-serif text-4xl tracking-tight text-white">Open the living desktop</h1><p className="mx-auto mt-4 max-w-lg text-sm leading-7 text-white/50">Open World’ni alohida full-screen tab oynasida ko‘ring. Sidebar yopiladi va daraxt atrofidagi barcha workspace’lar to‘liq ko‘rinadi.</p><button type="button" onClick={() => void openInNewWindow('/tools/world-3d', 'Yolnoma World')} className="mt-8 inline-flex items-center gap-2 rounded-2xl bg-amber-200 px-6 py-3.5 text-sm font-semibold text-slate-950 shadow-lg shadow-amber-200/10 transition hover:bg-amber-100"><Maximize2 size={17} /> Open 3D World</button></div></div>;
  }
