...
const LANE_COLORS = ['#22D3EE', '#A78BFA', '#F472B6', '#34D399', '#FBBF24', '#FB7185', '#60A5FA', '#C084FC'];
...
function curve(from: number, to: number, color: string, dashed = false) {
  const x1 = from * 30 + 18;
  const x2 = to * 30 + 18;
  const bend = x1 === x2 ? '' : ` C ${x1} 32, ${x2} 42, ${x2} 82`;
  return <path d={`M ${x1} 10 L ${x1} 28${bend || ` L ${x2} 82`}`} fill="none" stroke={color} strokeWidth={dashed ? 2 : 3} strokeLinecap="round" strokeDasharray={dashed ? '4 4' : undefined} opacity={dashed ? 0.55 : 0.95} />;
}
...
export default function CommitHistoryGraph({ folderPath, onFolderChange }: { folderPath: string; onFolderChange: (path: string) => void }) {
  ...
  const graphWidth = Math.max(74, (graph.maxLane + 1) * 30 + 24);
  ...
  return <ToolCard>
    ...
    {!loading && graph.rows.length > 0 && <div className="mt-8 overflow-auto border border-white/[0.08] bg-[#11101b] p-4"><div className="min-w-[760px]">{graph.rows.map((row) => <div key={row.commit.hash} className="group grid grid-cols-[auto_minmax(0,1fr)] gap-3" style={{ minHeight: 84 }}><div className="relative shrink-0" style={{ width: graphWidth }}><svg className="absolute inset-0 h-full overflow-visible" width={graphWidth} viewBox={`0 0 ${graphWidth} 84`} aria-hidden="true">{row.before.map((hash, beforeLane) => { const afterLane = row.after.indexOf(hash); return afterLane >= 0 && hash !== row.commit.hash ? curve(beforeLane, afterLane, laneColor(beforeLane), beforeLane !== afterLane) : null; })}{row.commit.parents.map((parent, parentIndex) => { const targetLane = row.after.indexOf(parent); return targetLane >= 0 ? curve(row.lane, targetLane, laneColor(parentIndex === 0 ? row.lane : row.lane + parentIndex), parentIndex > 0) : null; })}<circle cx={row.lane * 30 + 18} cy="10" r={row.commit.parents.length > 1 ? 6 : 5} fill={laneColor(row.lane)} stroke="#11101b" strokeWidth="3" /></svg></div><div className="min-w-0 border-b border-white/[0.06] pb-5 pt-0.5 group-last:border-0"><div className="flex flex-wrap items-center gap-2">{row.commit.parents.length > 1 ? <GitMerge size={14} className="text-amber-300" /> : <GitCommitHorizontal size={14} style={{ color: laneColor(row.lane) }} />}<code className="text-xs" style={{ color: laneColor(row.lane) }}>{row.commit.short_hash}</code>{row.commit.refs.map((ref) => <span key={ref} className="border px-2 py-0.5 text-[10px]" style={{ color: laneColor(row.lane), borderColor: `${laneColor(row.lane)}55`, backgroundColor: `${laneColor(row.lane)}18` }}>{ref}</span>)}{row.commit.parents.length > 1 && <span className="text-[10px] uppercase tracking-wider text-amber-200/70">merge · {row.commit.parents.length} parents</span>}</div><p className="mt-2 text-sm font-medium text-white/85">{row.commit.subject}</p><div className="mt-2 flex flex-wrap items-center gap-4 text-[11px] text-white/35"><span className="inline-flex items-center gap-1"><UserRound size={12} /> {row.commit.author}</span><span className="inline-flex items-center gap-1"><CalendarDays size={12} /> {row.commit.date}</span><span className="font-mono">{row.commit.hash.slice(0, 12)}</span></div></div></div>)}</div></div>}
