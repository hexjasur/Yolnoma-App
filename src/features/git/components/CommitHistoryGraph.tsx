...
export default function CommitHistoryGraph({ folderPath, onFolderChange }: { folderPath: string; onFolderChange: (path: string) => void }) {
  ...
  return <ToolCard>
    ...
    {!loading && rows.length > 0 && <div className="mt-8 overflow-hidden border border-white/[0.08] bg-[#11101b]"><div>
      ...
      <div className="px-4 py-2">{rows.map((row) => <div key={row.commit.hash} className="group grid min-h-[68px] grid-cols-[96px_minmax(0,1fr)_100px_120px_78px] items-start border-b border-white/[0.06] py-2 last:border-0"><div className="relative h-[60px] shrink-0" style={{ width: graphWidth }}><svg className="absolute inset-0 h-full overflow-visible" width={graphWidth} viewBox={`0 0 ${graphWidth} 68`} aria-hidden="true">{row.before.map((hash, beforeLane) => { const afterLane = row.after.indexOf(hash); return afterLane >= 0 && hash !== row.commit.hash ? curve(beforeLane, afterLane, laneColor(beforeLane), beforeLane !== afterLane) : null; })}{row.commit.parents.map((parent, parentIndex) => { const targetLane = row.after.indexOf(parent); return targetLane >= 0 ? curve(row.lane, targetLane, laneColor(parentIndex === 0 ? row.lane : row.lane + parentIndex), parentIndex > 0) : null; })}<circle cx={visualLane(row.lane) * LANE_WIDTH + 12} cy="8" r={row.commit.parents.length > 1 ? 5 : 4} fill={laneColor(row.lane === 0 ? 0 : row.lane)} stroke="#11101b" strokeWidth="2.5" /></svg></div><div className="min-w-0 pr-5"><div className="flex flex-wrap items-center gap-2">{row.commit.parents.length > 1 && <GitMerge size={13} className="text-amber-300" />}{row.commit.refs.map((ref) => <span key={ref} className="border px-2 py-0.5 text-[10px]" style={{ color: laneColor(row.lane), borderColor: `${laneColor(row.lane)}55`, backgroundColor: `${laneColor(row.lane)}18` }}>{ref}</span>)}{row.commit.parents.length > 1 && <span className="text-[10px] uppercase tracking-wider text-amber-200/70">merge · {row.commit.parents.length} parents</span>}</div><p className="mt-1 truncate text-sm font-medium text-white/85" title={row.commit.subject}>{row.commit.subject}</p></div><span className="truncate pt-1 text-xs text-white/45">{row.commit.date}</span><span className="truncate pt-1 text-xs text-white/55" title={row.commit.author}>{row.commit.author}</span><code className="pt-1 text-xs" style={{ color: laneColor(row.lane === 0 ? 0 : row.lane) }}>{row.commit.short_hash}</code></div>)}</div>
    </div></div>}
