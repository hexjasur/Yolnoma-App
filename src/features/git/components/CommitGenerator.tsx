...
function parseVariants(raw: string): CommitVariant[] {
  ...
  const variants = blocks
    .slice(0, 4)
    .map((message, index) => ({ title: VARIANT_TITLES[index], message }));
  ...
  return (
    <ToolCard>
      <ToolTitle
        ...
        subtitle="Generate four strong commit options from only the current modified and new files."
      />
      <div className="mt-6 flex flex-wrap items-center gap-3">
        <button
          type="button"
          onClick={() => void pickFolder()}
          className="inline-flex items-center gap-2 border border-[var(--accent)] bg-[var(--accent-glow)] px-4 py-2.5 text-sm font-semibold text-[var(--accent)] hover:bg-[var(--accent-dim)]"
        >
          <FolderOpen size={16} /> Select Git folder
        </button>
        {folderPath && (
          <button
            type="button"
            ...
            className="inline-flex items-center gap-2 border border-white/10 px-4 py-2.5 text-sm text-white/60 hover:text-white"
          >
            <RefreshCw
              size={15}
      ...
      {folderPath && (
        <p
          className="mt-4 truncate border border-white/[0.08] bg-black/20 px-4 py-3 font-mono text-xs text-white/55"
          title={folderPath}
        >
          {folderPath}
        </p>
      ...
      {loadingChanges && (
        <div className="mt-6 flex items-center gap-2 text-sm text-white/45">
          <Loader2 size={16} className="animate-spin" /> Reading Git changes...
        </div>
      )}
      {!loadingChanges && folderPath && !changes.length && !error && (
        <div className="mt-6 border border-dashed border-white/10 p-8 text-center text-sm text-white/40">
      ...
      {changes.length > 0 && (
        <div className="mt-6">
          ...
          <div className="max-h-52 overflow-auto border border-white/[0.08] bg-black/20">
            {changes.map((change) => (
              <div
              ...
              >
                ...
                />
                <span className="w-8 shrink-0 font-mono text-xs text-emerald-300/70">
                  {change.status || "M"}
                </span>
      ...
      {error && (
        <div className="mt-6 flex items-start gap-2 border border-red-400/20 bg-red-400/[0.06] p-4 text-sm text-red-200">
          <AlertCircle size={16} className="mt-0.5 shrink-0" /> {error}
        </div>
      )}
      <div className="mt-8 grid gap-4 border-t border-white/[0.08] pt-6 md:grid-cols-[minmax(0,1fr)_220px]">
        <div>
          <p className="text-xs text-white/45">
            The generator creates four perspectives: broad summary, technical
            detail, user impact, and a minimal clean version.
          </p>
          {fallbackNote && (
            <p className="mt-3 border-l-2 border-amber-300/70 pl-3 text-xs leading-5 text-amber-200/80">
              {fallbackNote}
            </p>
          )}
        </div>
        <div>
          <label className="block text-xs text-white/50">
            ...
            <select
              ...
              className="mt-2 w-full border border-white/10 bg-black/30 px-3 py-2.5 text-sm text-white outline-none"
      ...
      <div className="mt-5 flex flex-wrap items-center gap-4">
        <button
          ...
          className="inline-flex items-center gap-2 bg-[var(--accent)] px-5 py-3 text-sm font-semibold text-black transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-40"
        >
          {generating ? (
        ...
        >
          <KeyRound size={14} />{" "}
          {apiKey ? "Change API key" : "Add OpenRouter API key"}
        </button>
      </div>
      {variants.length > 0 && (
        <div className="mt-7 grid gap-4 lg:grid-cols-2">
          {variants.map((variant) => (
            <article
              key={variant.title}
              className="border border-white/[0.08] bg-black/20 p-5"
            >
              ...
              <textarea
                ...
                className="min-h-44 w-full resize-y border border-white/[0.08] bg-[#0d0d0a] p-4 font-mono text-xs leading-6 text-emerald-100/80 outline-none"
              />
            </article>
          ))}
        </div>
