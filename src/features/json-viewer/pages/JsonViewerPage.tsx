...
function createHtmlExport(value: JsonValue, title: string): string {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title || 'JSON export')}</title><style>body{margin:0;padding:32px;background:#14110e;color:#f2ede6;font:14px Inter,system-ui,sans-serif}.shell{max-width:1200px;margin:auto}.eyebrow{color:#d97757;text-transform:uppercase;letter-spacing:.16em;font-size:11px}.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:16px;margin-top:24px}.card{overflow:hidden;border:1px solid #40342c;border-radius:16px;background:#1c1713;box-shadow:0 12px 30px #0004}.card-image{width:100%;height:150px;object-fit:cover;background:#2a211c}.fields{padding:16px}.field{padding:9px 0;border-bottom:1px solid #332a24;display:flex;flex-direction:column;gap:4px}.field:last-child{border-bottom:0}.field span{font-size:10px;color:#a7988d;text-transform:uppercase;letter-spacing:.08em}.field strong{font-weight:500;overflow-wrap:anywhere}</style></head><body><main class="shell"><div class="eyebrow">Yolnoma JSON Viewer</div><h1>${escapeHtml(title || 'JSON export')}</h1><section class="grid">${cardHtml || '<p>No object records found.</p>'}</section></main></body></html>`;
}
    ...
    <div className="min-h-full pb-20" style={{ fontFamily: 'var(--font-sans)' }}>
      <div className="mb-8 flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <h1 className="font-serif text-4xl font-medium tracking-tight text-[var(--text-primary)]">JSON Viewer</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--text-muted)]">Load large JSON collections, rename keys, inspect image-rich records as cards, and export a clean JSON or standalone HTML view.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <label className="btn-secondary inline-flex cursor-pointer items-center gap-2"><Upload size={15} /> Load JSON<input type="file" accept=".json,application/json" onChange={handleFile} className="hidden" /></label>
          <button type="button" onClick={exportJson} disabled={parsed === null} className="btn-secondary inline-flex items-center gap-2 disabled:cursor-not-allowed disabled:opacity-40"><Download size={15} /> Export JSON</button>
          <button type="button" onClick={exportHtml} disabled={parsed === null} className="btn-primary inline-flex items-center gap-2 disabled:cursor-not-allowed disabled:opacity-40"><FileText size={15} /> Export HTML</button>
        </div>
      </div>
      ...
      <section className="mt-5 panel-card overflow-hidden">
        <div className="flex flex-col gap-4 border-b border-[var(--border)] px-5 py-4 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex items-center gap-3"><h2 className="font-semibold text-[var(--text-primary)]">Card preview</h2>{parsed !== null && <span className="rounded-full bg-[var(--accent-dim)] px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-[var(--accent)]">{cards.length.toLocaleString()} records</span>}</div><p className="mt-1 text-xs text-[var(--text-faint)]">{visibleCards.length < cards.length ? `Showing ${visibleCards.length.toLocaleString()} of ${cards.length.toLocaleString()} records` : 'Object records are rendered as visual cards.'}</p></div><div className="flex items-center gap-2"><div className="relative"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-faint)]" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Filter records..." className="form-input w-56 py-2 pl-9 text-xs" /></div><button type="button" onClick={() => setShowRaw((value) => !value)} className="btn-secondary inline-flex items-center gap-2 text-xs">{showRaw ? 'Cards' : 'Raw'} {showRaw ? <Check size={14} /> : <Braces size={14} />}</button></div></div>
