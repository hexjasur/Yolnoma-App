  ...
  const generate = async () => {
    ...
    try {
      ...
      for (const candidate of candidates) {
        ...
        if (candidate !== candidates[candidates.length - 1]) toast.info(`${getShortModelName({ id: candidate })} failed; testing the next model…`);
      }
  ...
  return <ToolCard>
    ...
    <div className="mt-5 grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
      <aside className="border border-white/[0.08] bg-black/10 p-4">
        ...
        {rootPath && <div className="mt-4 space-y-3"><div className="border border-white/[0.08] bg-white/[0.025] p-3"><p className="truncate text-xs font-semibold text-white">{projectName}</p><p className="mt-1 break-all text-[10px] leading-4 text-white/35">{rootPath}</p></div><div className="grid grid-cols-2 gap-2 text-center"><div className="border border-white/[0.07] p-2"><p className="text-lg font-semibold text-white">{fileCount}</p><p className="text-[10px] text-white/35">files found</p></div><div className="border border-white/[0.07] p-2"><p className="text-lg font-semibold text-white">{imageFiles.length}</p><p className="text-[10px] text-white/35">assets</p></div></div><label className="block text-[10px] text-white/35">OpenRouter model<select value={model} onChange={(event) => setModel(event.target.value)} className="mt-1 w-full border border-white/10 bg-[#181410] px-2 py-2 text-[11px] text-white outline-none">{models.map((item) => <option key={item.id} value={item.id}>{getShortModelName(item)}</option>)}</select></label>{activeModel && <p className="text-[10px] text-emerald-300/60">Used model: {getShortModelName({ id: activeModel })}</p>}<p className="text-[10px] leading-4 text-white/30">AI faqat Reads up to 36 text/config files relevant to the README; not all files are sent. Secret values ​​are not requested.</p></div>}
