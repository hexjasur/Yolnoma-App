...
const TEXT_FILE =
  /\.(md|mdx|txt|json|jsonc|js|jsx|mjs|cjs|ts|tsx|css|scss|html|xml|yaml|yml|toml|ini|env|rs|py|go|java|kt|swift|c|cpp|h|hpp|cs|php|rb|sh|sql|graphql|vue|svelte|astro)$/i;
...
async function describeAsset(path: string) {
  try {
    ...
    const dimensions = await new Promise<string>((resolve) => {
      ...
      image.onload = () => {
        resolve(`${size}, ${image.naturalWidth}×${image.naturalHeight}px`);
...
async function scanFolder(rootPath: string) {
  ...
  async function walk(current: string, depth: number) {
    ...
    try {
      ...
      if (depth === 0)
        throw new Error(
          `Selected folder could not be opened: ${String(error)}`,
        );
      return;
    }
...
async function requestReadme(
  ...
  const imageParts = assets
    ...
    .flatMap((asset) => [
      {
        type: "text",
        text: `Image candidate: ${asset.path} (${asset.details})`,
      },
...
export default function ReadmeGeneratorTool() {
  ...
  const projectName = useMemo(
    () => rootPath.split(/[\\/]/).filter(Boolean).pop() || "your project",
    [rootPath],
  );
  const imageFiles = useMemo(
    () =>
      entries
        .filter((entry) => entry.kind === "file" && IMAGE_FILE.test(entry.path))
        .map((entry) => relativePath(rootPath, entry.path)),
    [entries, rootPath],
  );
  ...
  const pickFolder = async () => {
    ...
    try {
      ...
      const textEntries = found.filter(
        (entry) => entry.kind === "file" && TEXT_FILE.test(entry.path),
      );
      const priority = textEntries.filter((entry) =>
        /(^|\/)(package\.json|readme|vite\.config|tsconfig|cargo\.toml|pyproject\.toml|requirements|dockerfile|\.env\.example|src\/|app\/|pages\/)/i.test(
          relativePath(selected, entry.path),
        ),
      );
      const selectedFiles = [
        ...priority,
        ...textEntries.filter((entry) => !priority.includes(entry)),
      ].slice(0, 36);
      let total = 0;
      ...
      for (const entry of selectedFiles) {
        ...
        try {
          const content = await invoke<string>("read_codebase_file", {
            rootPath: selected,
            relativePath: relativePath(selected, entry.path),
          });
          const chunk = content.slice(
            0,
            Math.min(MAX_FILE_CHARS, MAX_CONTEXT_CHARS - total),
          );
          snapshots.push(
      ...
      const tree = found
        .map(
          (entry) =>
            `${relativePath(selected, entry.path)}${entry.kind === "directory" ? "/" : ""}`,
        )
        .join("\n");
      const scannedImageFiles = found
        .filter((entry) => entry.kind === "file" && IMAGE_FILE.test(entry.path))
        .map((entry) => relativePath(selected, entry.path));
      const assetEntries = found
        .filter((entry) => entry.kind === "file" && IMAGE_FILE.test(entry.path))
        .slice(0, 20);
      const assetCandidates = await Promise.all(
        assetEntries.map(async (entry): Promise<AssetCandidate> => {
          ...
          try {
            ...
            const dataUrl =
              bytes.byteLength <= 2_000_000
                ? `data:${imageMime(entry.path)};base64,${toBase64(bytes)}`
                : undefined;
      ...
      const assetDetails = assetCandidates.map(
        (asset) => `${asset.path} — ${asset.details}`,
      );
      ...
      setDraftKey(key ?? "");
      if (key) {
        const foundModels = await fetchOpenRouterModels(key)
  ...
  const generate = async () => {
    ...
    if (!apiKey) {
      setError(
        "OpenRouter API key not found. Add your key in AI Chat or Agent settings.",
      );
    ...
    try {
      const candidates = [
        model,
        ...models.map((item) => item.id).filter((id) => id !== model),
      ];
      let response: ProxyResponse | null = null;
      let lastError = "";
      for (const candidate of candidates) {
        const attempt = await requestReadme(
          apiKey,
          candidate,
        ...
        if (
          attempt.status >= 200 &&
          attempt.status < 300 &&
          attempt.body.choices?.[0]?.message?.content
        ) {
          response = attempt;
        ...
        const canFallback =
          isLimitError(attempt.status, lastError) ||
          attempt.status >= 500 ||
          attempt.status === 400;
        if (!canFallback) throw new Error(lastError);
        if (candidate !== candidates[candidates.length - 1])
          toast.info(
            `${getShortModelName({ id: candidate })} failed; testing the next model…`,
          );
      }
      if (!response) throw new Error(`AI model failed: ${lastError}`);
      const content = response.body.choices?.[0]?.message?.content?.trim();
      if (!content) throw new Error("AI README not return.");
      setReadme(
        content
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
        <aside className="border border-white/[0.08] bg-black/10 p-4">
          ...
          <button
            ...
            className="mt-4 flex w-full items-center justify-center gap-2 border border-[var(--accent-border)] bg-[var(--accent-glow)] px-3 py-2.5 text-xs font-semibold text-white hover:bg-[var(--accent-dim)] disabled:opacity-50"
          >
            <FolderOpen size={15} />{" "}
            {loadingFolder
              ? "Scanning…"
          ...
          {rootPath && (
            <div className="mt-4 space-y-3">
              <div className="border border-white/[0.08] bg-white/[0.025] p-3">
                ...
                <p className="mt-1 break-all text-[10px] leading-4 text-white/35">
                  {rootPath}
                </p>
              </div>
              ...
              <label className="block text-[10px] text-white/35">
                ...
                <select
                  ...
                  className="mt-1 w-full border border-white/10 bg-[#181410] px-2 py-2 text-[11px] text-white outline-none"
                >
                  {models.map((item) => (
                    <option key={item.id} value={item.id}>
                      {getShortModelName(item)}
                    </option>
                  ))}
                </select>
              </label>
              {activeModel && (
                <p className="text-[10px] text-emerald-300/60">
                  Used model: {getShortModelName({ id: activeModel })}
                </p>
              )}
              <p className="text-[10px] leading-4 text-white/30">
                AI faqat Reads up to 36 text/config files relevant to the
                README; not all files are sent. Secret values ​​are not
                requested.
              </p>
            </div>
          )}
          {error && (
            <p className="mt-3 border-l-2 border-red-400/70 pl-3 text-xs leading-5 text-red-300">
              {error}
            </p>
          )}
        </aside>
        <section className="min-w-0 border border-white/[0.08] bg-[#0d0d0a]">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.08] px-4 py-3">
            <div>
              ...
              <p className="mt-1 text-xs text-white/30">
                AI first reads your project context and visual assets; always
                review before applying.
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <div className="inline-flex border border-white/10 bg-white/[0.03] p-0.5">
                <button
                  type="button"
                  onClick={() => setViewMode("preview")}
                  className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs ${viewMode === "preview" ? "bg-white/[0.10] text-white" : "text-white/40 hover:text-white"}`}
                >
                  <Eye size={14} /> Preview
                </button>
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs ${viewMode === "code" ? "bg-white/[0.10] text-white" : "text-white/40 hover:text-white"}`}
                >
                  <Code2 size={14} /> Code
                </button>
              </div>
              <button
                ...
                disabled={generating || loadingFolder || !rootPath}
                className="inline-flex items-center gap-1.5 bg-[var(--accent)] px-3 py-2 text-xs font-semibold text-[#1b120e] disabled:opacity-40"
              >
                {generating ? (
              ...
              {readme && (
                <button
                  type="button"
                  onClick={() => void applyReadme()}
                  disabled={applying}
                  className="inline-flex items-center gap-1.5 border border-emerald-400/30 bg-emerald-400/10 px-3 py-2 text-xs font-semibold text-emerald-200 disabled:opacity-40"
                >
                  {applying ? (
                    <Loader2 size={14} className="animate-spin" />
                  ) : (
          ...
          <div className="border-b border-white/[0.08] p-4">
            <label className="block text-[10px] font-semibold uppercase tracking-[0.16em] text-white/35">
              Custom instructions{" "}
          ...
          <div className="min-h-[450px] p-5">
            {readme ? (
              ...
              <div className="flex min-h-[400px] flex-col items-center justify-center text-center">
                <FileText size={32} className="text-[var(--accent)]/50" />
                <h3 className="mt-4 text-base font-semibold text-white/75">
                  A specific README, not a generic template
                </h3>
                <p className="mt-2 max-w-md text-xs leading-5 text-white/35">
                  Choose a project folder, add optional instructions, and
                  Yolnoma will inspect only the most relevant files and visual
                  assets before writing the documentation.
                </p>
              </div>
            )}
          </div>
