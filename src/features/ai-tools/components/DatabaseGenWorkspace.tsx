...
export default function DatabaseGenWorkspace() {
  ...
  return <section className="overflow-hidden rounded-3xl border border-white/[0.10] bg-[#100e0b] shadow-2xl">
    <div className="border-b border-white/[0.08] bg-gradient-to-r from-[#271b13] to-[#15110e] p-5">
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_190px_auto]"><textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} rows={2} placeholder="Example: Library Management System with members, books, loans, authors and categories" className="form-textarea resize-none border-white/10 bg-black/20 text-sm"/><select value={model} onChange={(event) => setModel(event.target.value)} className="border border-white/10 bg-[#181410] px-3 text-xs text-white outline-none"><option value={DEFAULT_MODELS[0].id}>{DEFAULT_MODELS[0].name}</option><option value={DEFAULT_MODELS[1].id}>{DEFAULT_MODELS[1].name}</option><option value={DEFAULT_MODELS[2].id}>{DEFAULT_MODELS[2].name}</option></select><button type="button" onClick={() => void generate()} disabled={generating} className="inline-flex items-center justify-center gap-2 bg-[var(--accent)] px-5 py-3 text-sm font-semibold text-[#1b120e] disabled:opacity-50">{generating ? <Loader2 size={16} className="animate-spin" /> : <Sparkles size={16} />} {generating ? 'Generating…' : 'Generate schema'}</button></div>
      <div className="mt-2 flex flex-wrap items-center gap-2"><button type="button" onClick={() => setPrompt(SAMPLE_PROMPT)} className="text-[11px] text-white/40 hover:text-white">Try Library Management System</button>{error && <span className="text-xs text-red-300">{error}</span>}</div>
    </div>
