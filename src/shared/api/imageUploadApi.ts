...
export default IMGBB_UPLOAD_URL;

if (!IMGBB_API_KEY) {
  console.warn(
    "[imgbb] VITE_IMGBB_API_KEY is not set — image uploads will fail.",
  );
}
...
export async function uploadImage(
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    xhr.addEventListener("error", () => {
      const msg = "Network error — image failed to load";
      emit({ phase: "error", progress: 0, error: msg });
      reject(new Error(msg));
    });
...
export function validateImageFile(file: File, maxMb = 32): string | null {
  ...
  if (file.size > maxMb * 1024 * 1024)
    return `The file size must not exceed ${maxMb} MB.`;
  return null;
}
