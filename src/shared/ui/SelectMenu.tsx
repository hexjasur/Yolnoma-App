  ...
  return (
    <div ref={rootRef} className={`relative ${className}`}>
      ...
      >
        ...
        <ChevronDown size={15} className={`shrink-0 text-[var(--text-muted)] transition-transform ${open ? 'rotate-180 text-[var(--accent)]' : ''}`} />
      </button>
      ...
      {open && (
        <div
          role="listbox"
          aria-label={ariaLabel}
          className="absolute right-0 z-50 mt-2 min-w-full overflow-hidden rounded-xl bg-[#211b17] p-1.5 shadow-[0_16px_40px_rgba(0,0,0,0.45)] ring-1 ring-white/10"
        >
          {options.map((option) => {
            ...
            return (
              <button
                ...
                className={`flex w-full items-center justify-between gap-4 rounded-lg px-3 py-2.5 text-left text-xs transition-colors ${active
